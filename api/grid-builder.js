...
// Returns: { pattern, across, down, fillTime }
//
// Core constraint-satisfaction engine for NYT-style 15×15 crosswords.
...
// set and the client can pass topic words via the prompt.

console.log("[grid-builder] Module loading...");
...
// Common crossword words (minimal set — in production, load from wordfreq)
const BASE_WORDLIST = {
  ...
  6: ["BEFORE", "CHANGE", "CALLED", "SHOULD", "PEOPLE", "NUMBER", "ALWAYS", "AROUND", "BECAME", "BETTER", "FATHER", "FOLLOW", "FRIEND", "CALLED", "MOTHER", "SECOND", "SYSTEM", "THOUGH", "DURING", "THINGS", "TURNED", "LIVING", "TAKING"],
  7: ["ANOTHER", "BETWEEN", "BECAUSE", "THROUGH", "PRESENT", "SEVERAL", "WITHOUT", "AGAINST", "GENERAL", "MORNING", "HIMSELF", "HERSELF", "NOTHING", "WORKING", "PICTURE", "PROBLEM", "SPECIAL", "FEELING", "REQUIRE", "SEVERAL", "CERTAIN", "ALREADY", "PERHAPS"],
  8: ["TOGETHER", "FINISHED", "BUSINESS", "CHILDREN", "SUDDENLY", "QUESTION", "BUILDING", "YOURSELF", "YOURSELF", "NATIONAL", "POSITION", "ALTHOUGH", "COMPLETE", "PERSONAL", "THINKING", "STRENGTH", "STRAIGHT", "STANDARD", "INTEREST", "CARRYING"],
};

console.log("[grid-builder] BASE_WORDLIST loaded");

// Build index: (length, position, letter) -> word IDs
function buildIndex(wordsByLength) {
  console.log("[grid-builder] buildIndex called");
  const idx = {};
...
function extractSlots(pattern) {
  ...
  for (let r = 0; r < size; r++) {
    ...
    for (let c = 0; c < size; c++) {
      if (pattern[r][c] === ".") {
        if (start !== null && c - start > 1) {
          slots.push(pattern[r].slice(start, c).split("").map((_, i) => [r, start + i]));
        }
        start = null;
      } else if (start === null) {
        start = c;
    ...
    if (start !== null && size - start > 1) {
      slots.push(pattern[r].slice(start).split("").map((_, i) => [r, start + i]));
    }
  }
...
// Solver: least-constraining-value + forward checking
    ...
    const doSolve = () => {
      ...
      for (const [_, wi] of scored.slice(0, 400)) {
        ...
        for (const [r, c] of slot) {
          const key = `${r},${c}`;
          saved[key] = grid[key];
          grid[key] = w[slots[si].indexOf([r, c])];
        }
