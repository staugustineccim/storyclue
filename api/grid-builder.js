// Massive expanded word list for better solver coverage
const BASE_FILLER = {
...
function solve(pattern, slots, wordsByLength, seed = 0, timeLimit = 12) {
  const t0 = Date.now();
  const grid = {};
  const assignment = {};
  const used = new Set();
...
export default async function handler(req, res) {
  ...
  const { pattern, slots, topicWords = [], seed = 0, timeLimit = 12 } = req.body;
